...
public class FlightTableController {
    ...
    // UI elements defined in the Dashboard
    ...
    // The main data list that holds all flight from the database
    private ObservableList<Flight> masterData = FXCollections.observableArrayList();

    
    // Constructor: Initialzies the UI components and triggers the first data load
    public FlightTableController(TableView<Flight> table,
        ...
        // Formating the statistics text area
        statsLabel.setMaxWidth(Double.MAX_VALUE);
        statsLabel.setWrapText(true);
        statsLabel.setPrefHeight(220);
        
        // Initial setup sequence 
        setupColumns();   // create table columns
        loadData();   // Get data from DB
        setupListeners();    // Add search and filter logic
        setupRowClick();    // Add clikc and hover effect
        updateAll(masterData);    //Build charts and stats
        refreshData();    // Initial UI reset
    }

    // Defines table columns and links them to flight model properties
    private void setupColumns() {
        ...
        // Custom Cell Factory: Colors Seat Types (Business = Green, First Purple)
        ...
        // Custom Cell Factory: Colors Status (On Time = Green, Delayed = Orange, Cancelled = Red)
        ...
        // Conditional Row Styling: Highlights Delayed Flights in light yellow
    ...
    // Helper to create basic text columns quickly 
    ...
    // Resets filters and reloads data while measuring system latency
    public void refreshData() {
      ...
      latencyLabel.setText("Latency: " + duration + " ms");
      lastUpdatedLabel.setText("Last Updated: " + java.time.LocalTime.now().withNano(0));
      
      System.out.println("[METRIC] Refresh Latency: " + duration + "ms");
    }
    
    // Exports currently visible table data to a CVS file
    public void exportToCSV() {
      long startTime = System.currentTimeMillis(); // Start Metric
      
      // 1. Get the current items from the table (respects your filters)
      List<Flight> dataToExport = table.getItems();
      
      // 2. Define the file path
      java.io.File file = new java.io.File("flight_export.csv");


      try (java.io.PrintWriter writer = new java.io.PrintWriter(file)) {
          // Write CSV Header
          writer.println("Flight,Airline,Origin,Destination,Status,Delay(Mins),Type");


          // Write Data Rows
          for (Flight f : dataToExport) {
          ...
          long duration = System.currentTimeMillis() - startTime; // End Metric
          ...
          // Log the automation metric 
          System.out.println("[METRIC] Export Time: " + duration + "ms");
    ...
    // Connects to SQLite and produce the master list and airport filter
    private void loadData() {
        try {
            ...
            // Build unique airport list for the dropdown filter
    ...
    // Attaches listeners to UI controls to trigger filtering on user input 
    ...
    // Filters the masters list based on search text and dropdown selections
    private void filterData() {
      ...
      List<Flight> filtered = masterData.stream()
              ...
              .filter(f -> route.equals("All") || f.getFlightType().equalsIgnoreCase(route)) 
      ...
      latencyLabel.setText("Latency: " + (endTime - startTime) + " ms"); 

    }

    // Master update function for all dashboard visualizations
    ...
    // Shows a popup window listing the top 5 flights with the highest delays
    public void showTopDelayedPopup() {
      // Sort and get top 5
      String top5Text = masterData.stream()
          .filter(f -> f.getDelayMinutes() > 0)
          .sorted((f1, f2) -> Integer.compare(f2.getDelayMinutes(), f1.getDelayMinutes()))
          .limit(5)
          .map(f -> String.format("• Flight %s (%s): %d min delay", 
                   f.getFlightNumber(), f.getAirline(), f.getDelayMinutes()))
          .collect(Collectors.joining("\n\n"));

      // Create the Popup
      Alert alert = new Alert(Alert.AlertType.INFORMATION);
      alert.setTitle("⚠️ Top 5 Delayed Flights");
      alert.setHeaderText("Current Highest Operational Delays");
      if (top5Text.isEmpty()) {
          top5Text = "No delayed flights found in current data.";
      }
      Label content = new Label(top5Text);
      content.setStyle("-fx-font-size: 14px; -fx-padding: 10;");
      alert.getDialogPane().setContent(content);
      alert.showAndWait();
    }

    // Calculates flight metrics using StatsService and updates the summary TextArea
    private void updateStats(ObservableList<Flight> data) {
      StatsService service = new StatsService();
      int total = service.getTotalFlights(data);
      double avgDelay = service.getAverageDelay(data);
      long delayed = service.getDelayedFlights(data);
      long cancelled = service.getCancelledFlights(data);
      
      long onTime = total - delayed - cancelled;
      double onTimePercent = total > 0 ? (onTime * 100.0) / total : 0;
      long nonStop = data.stream().filter(f -> f.getFlightType().equalsIgnoreCase("Non-stop")).count();
      long layover = data.stream().filter(f -> f.getFlightType().equalsIgnoreCase("Layover")).count();
    ...
    // Updates pieChart to show distribution of flight statuses
    ...
    //Updates BarChart to show total delay minutes aggregated by Airline
    ...
    // Loads local images based on the destination airport code
    ...
    // Helper to translate airport codes to city names for UI labels
    ...
    // Configures Tooltips, Row Hovering, and Double-click details popup
    private void setupRowClick() {
      table.setRowFactory(tv -> {
          TableRow<Flight> row = new TableRow<>();
          Tooltip tooltip = new Tooltip();

          // Handles dynamic row styling and tooltips content 
          row.itemProperty().addListener((obs, oldItem, newItem) -> {
              if (newItem == null) {
                  row.setTooltip(null);
                  row.setStyle("");
          ...
          // Show detailed info dialog on double click
